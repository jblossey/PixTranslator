...
/**
 * Exception thrown when an invalid window start is found.
 *
 *
 * @author C. Berens
 * @package PEL
 * @subpackage Exception
 */
class PelJpegInvalidWindowStartException extends PelException {

    /**
     * Construct a new invalid window start exception.
     *
     * The exception will contain a message describing the error,
     * including the window start value found.
     *
     * @param int the window_start in the data.
     */
    function __construct($window_start) {
        parent::__construct('Invalid value for window start: %d',
        $window_start);
    }
}
...
class PelJpeg {
    ...
    function load(PelDataWindow $d) {
	    ...
	    try {
            	...
            	throw new PelJpegInvalidWindowStartException($i+1);
	    }
